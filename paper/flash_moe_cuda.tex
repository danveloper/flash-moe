...
KTransformers~\cite{ktransformers} demonstrates CPU/GPU hybrid inference for MoE models, achieving $\sim$14~tok/s on Qwen3-235B with AMX-optimized CPU kernels for expert computation and GPU for attention. This approach avoids I/O entirely when experts fit in RAM but requires 384\,GB of system memory. At our target configuration (64\,GB RAM), KTransformers would rely on mmap paging, negating its CPU compute advantage. MoE-Lightning~\cite{moelightning} introduces paged weight management for throughput-oriented (batched) serving, whereas our system targets single-request latency. MoE-Gen~\cite{moegen} batches expert computations across requests to improve GPU utilization, complementary to our single-request streaming approach. FloE~\cite{floe} proposes on-the-fly expert loading with adaptive scheduling for memory-constrained GPUs; our three-tier caching hierarchy addresses a similar constraint but adds the SSD tier for models exceeding total DRAM capacity. FlexGen~\cite{flexgen} demonstrated CPU--GPU--SSD offloading for dense models but does not address MoE expert sparsity. DeepSpeed-MoE~\cite{deepspeedmoe} provides expert parallelism across multiple GPUs in data center settings; our work targets consumer hardware with a single GPU.
...
The three-tier caching approach generalizes to any MoE model where experts dominate total parameters. DeepSeek-V3~\cite{deepseekv3} (671B, 37B active) at 2-bit expert quantization would produce $\sim$200\,GB of expert data, within range of our streaming approach. The architecture-agnostic design (compile-time configuration via model manifest) has been validated on Qwen3.5-397B; adapting to Mixtral~\cite{mixtral} or other MoE models requires only re-running the weight extraction and repacking pipeline.
...
We demonstrate that Mixture-of-Experts models exceeding 4$\times$ DRAM capacity can run at interactive speeds on commodity NVIDIA GPUs through a three-tier expert caching hierarchy. The key insight is that discrete GPU memory, while a liability for streaming, becomes an asset when used as a high-bandwidth expert cache. On RTX~4090, our system achieves $5.57 \pm 0.12$~tok/s sustained on Qwen3.5-397B-A17B, 28\% faster than the original Apple Silicon implementation, using only 16\,GB of system RAM. The complete engine is two source files totaling $\sim$2500 lines of CUDA/C, with no framework dependencies.

% ============================================================================
\paragraph{AI Disclosure.} This paper was co-authored with Claude Opus 4.6 (Anthropic), which implemented the CUDA inference engine, kernel optimizations, and server infrastructure through interactive pair-programming sessions directed by the first author. Claude also assisted with drafting and revising sections of this manuscript. All experimental design, measurements, and analysis were conducted by the first author.
