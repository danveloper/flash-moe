...
The three-tier caching approach generalizes to any MoE model where experts dominate total parameters. DeepSeek-V3~\citep{deepseekv3} (671B, 37B active) at 2-bit expert quantization would produce $\sim$200\,GB of expert data, within range of our streaming approach. The architecture-agnostic design (compile-time configuration via model manifest) has been validated on Qwen3.5-397B; adapting to Mixtral~\citep{mixtral} or other MoE models requires only re-running the weight extraction and repacking pipeline.
